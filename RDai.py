...
#hw2 prompt change info
    ...
    prompt = (
    "你是一位醫療對話分析專家，請根據以下編碼規則評估病人與醫生的每一句對話，\n"
    "每個項目若觸及則標記為 1，否則留空。請依據以下項目評估對話內容：\n"
    "\n".join(ITEMS) +
...
def main():
    ...
    for start_idx in range(0, total, batch_size):
        ...
        if len(batch_results) < len(batch):
            batch_results.extend([{item: "" for item in ITEMS}] * (len(batch) - len(batch_results)))
